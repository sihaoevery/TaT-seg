...
class Trainer(object):
    ...
    #     file_handler = logging.FileHandler('%s/%s.txt' % (self.saver.experiment_dir, 'log'))
    #     file_handler.setLevel(logging.INFO)
    #     file_handler.setFormatter(formatter)
    #     logger.addHandler(file_handler)
    #     return logger
...
def main():
    ...
    end_time = time.time()
    trainer.logger.info('===> Finished! Total time cost {}'.format(time.strftime("%H:%M:%S",time.gmtime(end_time-start_time))))
